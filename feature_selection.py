...
def forward_selection(max_features, ml_models, patient_x, patient_y):
    ...
    for i in range(0, max_features):
        ...
        for feature in features_left:
            ...
            fet_patient_x = patient_x[temp_features]
            models = make_algorithms(sample_models)
            performance = models[0]['score']
            performace = next(item for item in performance if item.get('mae'))['mae']
...
# TODO: Needs to be tested
def correlate_features(max_features, ml_models, patient_x, patient_y):
    correlations = []
